...
export async function getCategoryBySlug(
  ...
  return get_api(
    `https://localhost:7247/api/categories/${urlSlug}?PageSize=${pageSize}&PageNumber=${pageNumber}`
  );
}
...
export async function updateCategory(id = 0, formData) {
  return put_api(`https://localhost:7247/api/categories/${id}`, formData);
}
